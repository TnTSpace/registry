...
<!-- <div>
	<div class="flex items-center gap-4 py-4">
		<Input
			...
			value={(dt.table.getColumn('email')?.getFilterValue() as string) ?? ''}
			oninput={(e) => dt.oninput(e)}
			onchange={(e) => dt.onchange(e)}
		/>
		<DropdownMenu.Root>
			...
			<DropdownMenu.Content align="end">
				{#each dt.table.getAllColumns().filter((col) => col.getCanHide()) as column (column.id)}
	...
	<div class="rounded-md border">
		<Table.Root>
			<Table.Header>
				{#each dt.table.getHeaderGroups() as headerGroup (headerGroup.id)}
					<Table.Row>
						{#each headerGroup.headers as header, i}
							{#if i === headerGroup.headers.length - 1}
								<Table.Head class="justify-end text-right">
								...
								<Table.Head class="space-x-0.5">
									{#if !header.isPlaceholder}
										<FlexRender
											...
											context={header.getContext()}
										/>
										<Button size="icon" variant="outline" disabled={dt.selectedRows.length === 0}>
											<Trash2Icon class="size-4" />
										</Button>
									{/if}
								</Table.Head>
			...
			<Table.Body>
				{#each dt.table.getRowModel().rows as row (row.id)}
					{@const original = row.original as Record<string, any>}
					{@const keys = Object.keys(original as any)}
					<Table.Row class="relative" data-state={row.getIsSelected() && 'selected'}>
						{#each row.getVisibleCells() as cell, i}
							{#if i === 0}
								<Table.Cell class="flex items-center">
									...
									<Button
										size="icon"
										variant="ghost"
										onclick={() => dt.selectRow(row.id)}
										class="md:hidden"
									>
										{#if dt.isSelected(row.id)}
											<ChevronDownIcon class="size-4" />
										{:else}
											<ChevronRightIcon class="size-4" />
										{/if}
					...
					<tr class={cn('hidden', dt.isSelected(row.id) && 'table-row', 'lg:hidden')}>
						<td colspan="3">
							{#each keys as key, i}
								{#if dt.bp === 'xs'}
									{#if key !== dt.getColumnKeys()[1]}
										<div class="flex h-10 items-center gap-4 border-b p-2">
											<div class="font-bold">
												{key}
											</div>
									...
									{/if}
								{:else if dt.bp === 'sm'}
									{#if key !== dt.getColumnKeys()[1] && key !== dt.getColumnKeys()[2]}
										<div class="flex h-10 items-center gap-4 border-b p-2">
											<div class="font-bold">
												{key}
											</div>
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-2 py-4 text-sm text-muted-foreground">
			{dt.table.getFilteredSelectedRowModel().rows.length} of{' '}
			{dt.table.getFilteredRowModel().rows.length} row(s) selected.
		</div>
		<div class="flex items-center gap-2 py-4">
			<Button
				variant="outline"
