<script lang="ts">
	...
	import type { iDocument, iDocumentCategory } from '$lib/interface';
...
const getDocumentsAtIndex = (docGroups: [string, iDocument[] | undefined][], index: number): iDocument[] => {
...
const findDocumentInGroup = () => {
	...
	for (let index = 0; index < docGroups.length; index++) {
		...
		for (let i = 0; i < documents.length; i++) {
			...
			prev = isFirst ? obtainPrevFromFirstDocument({ docGroups, index }) : documents[i - 1] ?? null;
			next = isLast ? obtainNextFromLastDocument({ docGroups, index }) : documents[i + 1] ?? null;
